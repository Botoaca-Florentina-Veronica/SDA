...
void quicksort(echipa echipe[], int prim, int ultim)
{
	...
	{
		while (strcmp(echipe[stanga].nume , echipe[pivot].nume) < 0)
		{
			stanga++;
		}
		while (strcmp(echipe[dreapta].nume,  echipe[pivot].nume) > 0)
		{
			dreapta--;
		}
		if (stanga < dreapta)
...
void mutareEchipe(echipa echipe[], int N)
{
	...
	{
		while (echipe[pozPrim].puncte > 45)
		{
			pozPrim++;
		}
		while (echipe[pozUltim].puncte < 20)
		{
			pozUltim--;
		}
		if (pozPrim < pozUltim)
...
{
	int i, pozPrim, pozUltim;
	pozPrim = 0;
	pozUltim = N - 1;

	while (pozPrim <= pozUltim)
	{
		while (echipe[pozPrim].buget > 100000)
		{
			pozPrim++;
		}
	...
	// Apelam functia de sortare alfabetica pentru echipele cu buget mai mare decat 100,000
	quicksort(echipe, 0, pozPrim - 1);
}

int main(void)
{
	echipa echipe[] = { {"fcsb", 35, 15000, "Bucuresti"},
					   {"upt", 66, 2000, "Timisoara"},
					   {"cluj", 112, 400000, "Cluj-Napoca"},
					   {"dinamo", 3, 300000, "Ilfov"},
					   {"real madrid", 8, 10000, "Severin"} };
	int N, i;
